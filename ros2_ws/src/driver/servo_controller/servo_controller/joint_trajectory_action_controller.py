...
class JointTrajectoryActionController(Node):
    ...
    def follow_trajectory_callback(self, goal_handle):
        ...
        lookup = []
        for joint in self.joint_names:
            lookup.append(traj.joint_names.index(joint))  # 将joint的顺序转为数字索引(convert the order of joints to numerical indexes)
        ...
        for i in range(1, num_points):
            # 下一个减去上一个的时间戳算出来就是这个轨迹运行需要的时间(The difference between the timestamps of the next point and the previous point is the time required for the trajectory to run)
        ...
        for i in range(num_points):  # 遍历所有轨迹点，将他重新存储到列表里(traverse all trajectory points and store them in a list)
