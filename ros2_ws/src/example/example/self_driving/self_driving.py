    ...
    def main(self):
        while self.is_running:
            ...
            if self.start:
                ...
                if 450 < self.crosswalk_distance and not self.start_slow_down:  # 只有离斑马线足够近时才开始减速，通过判断斑马线的y轴像素距离(The robot starts to slow down only when it is close enough to the zebra crossing, which is determined by judging the y-axis pixel distance of the zebra crossing)
                ...
                if self.start_slow_down:
                    if self.traffic_signs_status is not None:
                        ...
                        if self.traffic_signs_status.class_name == 'red' and area < 1000:  # 如果遇到红灯就停车, 如果太近不停(If the robot detects a red traffic light, it will stop; if the distance is too close, it will not stop)
                            self.mecanum_pub.publish(Twist())
                            self.stop = True
                ...
                if 0 < self.park_x and 200 < self.crosswalk_distance:
                    twist.linear.x = self.slow_down_speed
                    if not self.start_park and 340 < self.crosswalk_distance:  # 离斑马线足够近时就开启停车(When the robot is close enough to the crosswalk, it will start parking)
                        self.mecanum_pub.publish(Twist())
                ...
                if self.turn_right:
                    ...
                    if not self.detect_far_lane:
                        ...
                        if 50 < center < 90:  # 当将要看不到车道线时切换到识别较远车道线(When the lane is about to be out of sight, switch to detecting the far lane)
