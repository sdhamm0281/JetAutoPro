...
class ActionGroupController:
    ...
    def run_action(self, action_name):
        '''
        运行动作组，无法发送stop停止信号(run action group without being able to send a stop signal)
        :param action_name: 动作组名字 ， 字符串类型(name of the action group, string type)
        :param times:  运行次数(number of times to run the action group)
        :return:
        '''
        ...
        if os.path.exists(action_name) is True:
            if self.running_action is False:
                ...
                while True:
                    ...
                    else:   # 运行完才退出(exit after running the program)
                        break
