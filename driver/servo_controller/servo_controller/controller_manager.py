...
class ControllerManager(Node):
    def __init__(self, name):
        rclpy.init()
        super().__init__(name, allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)  # 允许未声明的参数(allow undeclared parameter)
        ...
        # trajectory_controller的初始化(initialize trajectory_controller)
        self.controllers = {}
        connected_ids = {}
        ...
        # 实例化舵机管理节点(instantiate servo management node)
        self.servo_manager = ServoManager(connected_ids)
        self.servo_manager.connect()  # 检查是否有给定的舵机已连接(check if a given servo is connected)
        ...
        # 确保ros_robot_controller已完成初始化(make sure that ros_robot_controller has completed initialization)
