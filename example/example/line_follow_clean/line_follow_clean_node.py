...
# @data:2024/02/28
# @author:aiden
# 巡线清障(clear obstacle during line following)
import os
import cv2
import time
import math
...
MAX_SCAN_ANGLE = 240  # 激光的扫描角度,去掉总是被遮挡的部分degree(Laser scanning angle, removing the degree that is always obstructed)
...
class LineFollowCleanNode(Node):
    def __init__(self, name):
        ...
        self.create_service(Trigger, '~/start', self.start_srv_callback, callback_group=timer_cb_group) # 进入玩(enter the game)
    ...
    def lidar_callback(self, lidar_data):
        # 数据大小 = 扫描角度/每扫描一次增加的角度(data size = scanning angle / angle increment per scan)
        ...
        # 根据设定取数据(obtain data based on the set)
