            ...
            for i in self.target_colors:
                if self.detect_type[i] == 'line' and i != '':
                    ...
                    for roi in self.line_roi:
                        ...
                        max_contour_area = common.get_area_max_contour(contours, 0)[0]  # 获取最大面积对应轮廓(find the largest contour)
                        if max_contour_area is not None:
                            rect = cv2.minAreaRect(max_contour_area)  # 最小外接矩形(find the maximum area)
                            box = np.intp(cv2.boxPoints(rect))  # 四个角(four points)
                            for j in range(4):
